...
public partial class MainWindow : Window
{
    ...
    private void ComputeBtn_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (!double.TryParse(FreqGHzBox.Text, NumberStyles.Float, CultureInfo.InvariantCulture, out var fGHz) || fGHz <= 0)
        {
            MessageBox.Show("Невірна частота.");
            return;
        }
        var freqHz = fGHz * 1e9;
        var clearancePct = 60.0;
        if (double.TryParse(ClearancePctBox.Text, NumberStyles.Float, CultureInfo.InvariantCulture, out var cp) && cp > 0)
            clearancePct = cp;
